...
{
    public class SurNameStep : IStep<BotExampleContext>, IUpdateHandler<BotExampleContext>
    {
        private readonly FormRepository _formRepository;
        private readonly MessageLocalizationRepository _messageLocalization;

        public SurNameStep(FormRepository formRepository, MessageLocalizationRepository messageLocalization)
        {
        ...
        public async Task HandleAsync(BotExampleContext context, UpdateDelegate<BotExampleContext> prev, UpdateDelegate<BotExampleContext> next, CancellationToken cancellationToken)
        {
            if (On.Message(context))
            {
                ...
                context.UserState.CurrentState.CacheData =
                    context.UserState.CurrentState.CacheData.AddProperty<AuthorizationModel>(context.Update.Message.Text, nameof(AuthorizationModel.LastName));
                ...
                messageBuilder.AppendLine($"<b>{_messageLocalization.GetMessage("authorization.form.surname")}</b>: {context.Update.Message.Text}");
