...
{
    public class SelectMultipleElements<TModel, TItem, TContext> : BaseSelectElement<TModel, TItem, TContext>, ICallbackButtonHandler<TContext>, IUpdateHandler<TContext>, IStep<TContext>
...

    {
        public async override Task HandleAsync(TContext context, UpdateDelegate<TContext> prev, UpdateDelegate<TContext> next, CancellationToken cancellationToken)
        {
            if (context.Update.Type == UpdateType.CallbackQuery)
            {
                ...
                {
                    context.UserState.CurrentState.CacheData = context.UserState.CurrentState.CacheData.AddItemToArray<TModel>(propertyValue, PropertyName);
