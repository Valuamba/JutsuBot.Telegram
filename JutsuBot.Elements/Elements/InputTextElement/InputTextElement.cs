...
namespace CliverBot.Console.Form.v3.Elements
{
        ...
        where TModel : new()
    {
            ...
            if (context.Update.Type == UpdateType.Message)
            {
                if (ValidationHandlers != null)
                {
                    foreach (var validationHandler in ValidationHandlers)
                    {
                        ...
                        {
                            await context.InputTextClient.SendValidationInfo(context.Update.GetSenderId(), validationHandler.ErrorMessageAlias);
                            return;
                        }
                    }
                }
                ...
                await context.InputTextClient.StorePropertyInCache<TModel>(context, context.Update.Message?.Text, LocalizationSettings, PropertyName);
        ...
        {
            context.UserState.CurrentState.Step++;
            await context.InputTextClient.SendInformationMessage(context, PropertyName, LocalizationSettings, replyMarkup: EntryReplyKeyboardMarkup);
        }
