...
namespace JutsuBot.Elements
{
    ...
    {
        public static bool IsStage(this string route, string stage)
        {
            return route.StartsWith(stage);
        }

        public static Dictionary<string, object> Parse(this string route)
        {
            Dictionary<string, object> routeParamters = new();

            var match = new Regex(@"(?<stage_name>\w+\d*)(\s|\/)(?<parameters>((?<param_name>\w+)=(?<param_value>\w*\d*(&|\b)))+)").Match(route);
            ...
            parameters.Split('&').ToList().ForEach(
                p =>
                {
                    var separateIndex = p.IndexOf('=');

                    var paramterName = p.Substring(0, separateIndex);
                    var paramterValue = p.Substring(separateIndex + 1, p.Count() - separateIndex - 1);

                    routeParamters.Add(paramterName, paramterValue);
                });

            return routeParamters;
        }
